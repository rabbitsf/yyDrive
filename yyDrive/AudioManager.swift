...
class AudioManager: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    func startPlayback(folder: String, song: String, fileManager: FileManagerHelper) {
        folderPath = fileManager.getFolderPath(folder)
        songs = fileManager.getSongs(in: folder)
    ...
    // New method to support path-based navigation (for yyDrive)
    func startPlayback(path: String, song: String, fileManager: FileManagerHelper) {
        folderPath = fileManager.getBasePath() + "/" + path
        songs = fileManager.getSongs(inPath: path)
        ...
        if let index = songs.firstIndex(of: song) {
            ...
            playSong(from: playbackPositions[song] ?? 0)
        }
    }
