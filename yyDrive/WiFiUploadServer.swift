...
class WiFiUploadServer: ObservableObject {
    ...
    @objc private func appWillEnterForeground() {
        endBackgroundTask()
        // Restart server if it should be running but isn't
        if shouldKeepRunning && !isServerRunning {
            startServer()
        }
    }
    ...
    func startServer() {
        ...
        // Prevent screen from locking while server is running (optional, but helps)
        ...
        do {
            ...
            // Keep connection alive even when app goes to background
            parameters.includePeerToPeer = false
            parameters.requiredInterfaceType = .wifi
            
            // Configure TCP options for keepalive to maintain connections in background
            ...
            listener?.stateUpdateHandler = { [weak self] state in
                DispatchQueue.main.async {
                    ...
                    case .ready:
                        ...
                        print("✅ WiFi Upload Server started on port \(self.port)")
                    case .failed(let error):
                        print("❌ Server failed: \(error)")
                        self.isServerRunning = false
                        // Attempt to restart if failed and should keep running
                        if self.shouldKeepRunning {
                            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                                if !self.isServerRunning && self.shouldKeepRunning {
    ...
    private func startBackgroundTask() {
        ...
        backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "WiFiUploadServer") { [weak self] in
            self?.endBackgroundTask()
            // If server should keep running, try to restart
            if let self = self, self.shouldKeepRunning {
                print("⚠️ Background task expiring, attempting to keep server alive...")
    ...
    private func handleNewConnection(_ connection: NWConnection) {
        ...
        // Start background task when new connection arrives
        startBackgroundTask()
    ...
    private func handleUpload(data: Data, connection: NWConnection) {
        ...
        // Get the file manager helper
        guard let fileManagerHelper = getFileManagerHelper() else {
            sendResponse(connection: connection, statusCode: 500, body: "File manager not available")
            return
        }
        
        var successCount = 0
        for (filename, fileData) in files {
            let folderPath = fileManagerHelper.getFolderPath(folder)
    ...
    private func handleCreateFolder(data: Data, connection: NWConnection) {
        ...
        do {
            try fileManagerHelper.createFolder(name: folderName)
    ...
    // Helper to get FileManagerHelper instance
    private func getFileManagerHelper() -> FileManagerHelper? {
        // Access the shared instance through the app
        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let window = windowScene.windows.first,
           let rootView = window.rootViewController?.view {
            // Try to find FileManagerHelper from environment
            // For simplicity, we'll create a new instance since it's based on Documents directory
    ...
    private func sendHTMLResponse(connection: NWConnection) {
        ...
        let folders = fileManagerHelper.getFolders()
    ...
    private func generateHTML(folders: [String]) -> String {
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>📁 yyDrive Upload</h1>
                    <p>Upload files to your yyDrive folders</p>
                </div>
                <div class="content">
                    <div class="section">
